...
calculateTriangleNormals(std::vector<IndicesTriangle> const &indexTriangles, std::vector<math::Vec3f > const &vertices) {
...
calculateVertexNormals(std::vector<IndicesTriangle> const &indexTriangles,
	...
	for each (auto tNorms in vertStorage)
	{
		math::Vec3f v;
		int count = 0;
		for each (auto vi in tNorms)
		{
			v += (vi);
			count++;
		}
